...
void lex_error(char c, long i) {
  ...
  if (c == '\n')
    printf("Compilation Failed [LEXER] (%ld): unexpected char NEWLINE\n", i);
  else
    printf("Compilation Failed [LEXER] (%ld): unexpected char %c\n", i, c);
  exit(EXIT_FAILURE);
}
...
void parse_error(token *t) {
  ...
  printf("Compilation Failed [PARSER] (%ld): unexpected token of type %s\n",
         t->start, token);
  exit(EXIT_FAILURE);
}
...
void typecheck_error(char *msg, long i) {
  if (verbose)
    print_src_location(i);
  printf("Compilation failed [TYPECHECKER] (%ld): %s\n", i, msg);
  exit(EXIT_FAILURE);
}
...
void print_src_location(long i) {
  ...
  loc->line = line;
  long print_start = line - 1 < 1 ? 1 : line - 1;

  long cur_line = 1;
  printf("%ld\t", print_start);
  for (long j = 0; j < strlen(src); j++) {
    if (cur_line == print_start) {
      long print_count = 0;
      for (long k = j; k < strlen(src); k++) {
        if (print_count == 1)
          printf("\033[31m");
        printf("%c", src[k]);
        printf("\033[0m");
        if (src[k] == '\n') {
          ...
          if (print_count == 3 || k == strlen(src) - 1)
            break;
          printf("%ld\t", print_start + print_count);
        }
      }
      break;
    } else if (src[j] == '\n') {
